...
public class QuotaStatsServiceImpl extends DefaultComponent implements QuotaStatsService {
    ...
    public void start(ComponentContext context) {
        ...
        this.<QuotaStatsUpdaterDescriptor> getRegistryContributions(QUOTA_STATS_UPDATERS_EP)
            .stream()
            .forEach(contrib -> {
                try {
                    String name = contrib.getName();
                    QuotaStatsUpdater updater = contrib.getQuotaStatsUpdaterClass()
    ...
    public void updateStatistics(final DocumentEventContext docCtx, final Event event) {
        ...
        new UnrestrictedSessionRunner(docCtx.getCoreSession()) {
            @Override
            public void run() {
                for (QuotaStatsUpdater updater : getQuotaStatsUpdaters()) {
                    if (log.isTraceEnabled()) {
                        DocumentModel doc = docCtx.getSourceDocument();
                        log.trace("Calling updateStatistics of {} for {} on {} ({})", updater.getName(),
                                event.getName(), doc.getId(), doc.getPathAsString());
                    }
